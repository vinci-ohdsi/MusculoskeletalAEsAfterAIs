...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (45495534,45495533,4166080,4066890,4204075,4082236,4041195,4235010,4014640,2000217,4234291,21494055)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (45495534,45495533,4166080,4066890,4204075,4082236,4041195,4235010,4014640,2000217,4234291,21494055)
  and c.invalid_reason is null

) I
LEFT JOIN
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (4066326)
...
(
  select po.* 
  FROM @cdm_database_schema.PROCEDURE_OCCURRENCE po
JOIN #Codesets codesets on ((po.procedure_source_concept_id = codesets.concept_id and codesets.codeset_id = 9))
) C
JOIN @cdm_database_schema.PERSON P on C.person_id = P.person_id
WHERE YEAR(C.procedure_date) - P.year_of_birth >= 55
AND P.gender_concept_id in (8532)
